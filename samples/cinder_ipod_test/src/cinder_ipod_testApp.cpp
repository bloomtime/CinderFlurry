...
{
    Flurry::getInstrumentation()->init("Z3FYQ1Q8X49FDTIQY3LS");
    settings->enableMultiTouch(true);
}

void cinder_ipod_testApp::setup()
{
    Flurry::getInstrumentation()->startTimeEvent("loadMusic");
    vector<ipod::PlaylistRef> albums = getAlbums();
    Flurry::getInstrumentation()->stopTimeEvent("loadMusic");

    // Load Album Art
	int i = 0;
    Flurry::getInstrumentation()->startTimeEvent("loadArt");

    for(vector<PlaylistRef>::iterator it = albums.begin(); it != albums.end() && i++ < 64; ++it){
        PlaylistRef &album = *it;
        Flurry::getInstrumentation()->incrementActivity();
    ...
    Flurry::getInstrumentation()->stopTimeEvent("loadArt");
